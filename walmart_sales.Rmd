...
```{R}
kruskal.test(Weekly_Sales~Store, data = walmartNEW)
```
```{R}
library(FSA)
DT02 = dunnTest(Weekly_Sales~Store, data = walmartNEW,method="none")
DT02
```
...
```{r}
CRD_sales=aov(Weekly_Sales~Store, data = walmartNEW) #Perform ANOVA for CRD
#summary(CRD_sales)
#library(agricolae)
SNK = SNK.test(CRD_sales,"Store",group=TRUE)
print(SNK)
```
**Extract SNK results and lookup the Store Size**
```{r}
SNK = read.csv("C:/Users/Admin/My Drive/MDSA/Dataset/01. Supply Chain/Walmart/SNKresults.CSV")
head(SNK,3)
```

```{r}
SNK01 <- SNK[order(-SNK$Weekly_Sales), ]
library(ggplot2)
ggplot(SNK01, aes(x = reorder(Store, -Weekly_Sales), y = Weekly_Sales, fill = Type)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x = "Store", y = "Weekly Sales") +
  ggtitle("Weekly Sales by Store")
```
```{r}
SNK02 <- SNK[order(-SNK$Sales.per.sqf), ]
library(ggplot2)
ggplot(SNK02, aes(x = reorder(Store, -Sales.per.sqf), y = Sales.per.sqf, fill = Type)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  labs(x = "Store", y = "Weekly Sales") +
  ggtitle("Sales per SQF by Store")
```
